...
int main(int argc, char *argv[]) {
	if (!init())
		...
		if (!loadMedia())
			...
			initDot(dot, DOT_WIDTH / 2, DOT_HEIGHT / 2);

			Dot *otherDot = malloc(sizeof(Dot));
			initDot(otherDot, SCREEN_WIDTH / 4, SCREEN_HEIGHT /4);
			...
			while (!quit) {
				...
				moveDot(dot, &wall, getCollider(otherDot));
				...
				renderDot(otherDot, gDotTexture, gRenderer);
